...
//	FAnimationRuntime::ConvertBoneSpaceTransformToCS(Output.AnimInstanceProxy->GetComponentTransform(), Output.Pose, BS_OG_EndBoneCSTransform, IKBoneCompactPoseIndex, EBoneControlSpace::BCS_ComponentSpace);
	...
	UDragonIK_Library::Solve_Modified_Direct_TwoBoneIK_4(owning_skel->GetComponentTransform(), wrist_offset,UpperLimbCSTransform, LowerLimbCSTransform, EndBoneCSTransform, JointTargetPos, DesiredPos, bAllowHandStretching,1, HandData.Max_Stretch_Ratio,up_arm_twist_technique);
	...
	*/
	...
	// Update transform for end bone.
	{
		...
		//	FQuat Hand_Rotation_Temporary = ThighTransform.GetRotation();
			FQuat Hand_Rotation_Temporary = owning_skel->GetComponentToWorld().GetRotation().Inverse() * ThighTransform.GetRotation();

			if (rotation_method == ERotation_Type_DragonIK::ENUM_AdditiveRotation)
			{

				Hand_Rotation_Temporary = (ThighTransform.GetRotation() * EndBoneCSTransform.GetRotation());

			}

			EndBoneCSTransform.SetRotation( FQuat::Slerp(  EndBoneCSTransform.GetRotation(), Hand_Rotation_Temporary, arm_alpha));
