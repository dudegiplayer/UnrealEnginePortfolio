...
FAnimNode_DragonFeetSolver::FAnimNode_DragonFeetSolver()
{
	...
	Trace_Down_Multiplier_CurveData->AddKey(100.f, 0.f);
			...
			if (spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].bBlockingHit)
			{
				...
				if (spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].bBlockingHit && atleast_one_hit)
				{
					...
					if (Use_Four_Point_Feets && Is_Four_Point_Hitting)
					{
						...
						forward_final_impact = UKismetMathLibrary::InverseTransformDirection(owning_skel->GetComponentToWorld(), forward_final_impact);

						normal_impact = forward_final_impact;

						normal_impact = UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), normal_impact);
					}
				...
				{
					{
						...
						FVector up_vector_ws = UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), character_direction_vector_CS);
						...
						if (frontal_finger_hits && sideway_feet_hits)
						{
							...
							normal_finger_impact = UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), normal_finger_impact);
						...
						//forward_impact = UKismetMathLibrary::InverseTransformDirection(owning_skel->GetComponentToWorld(), forward_impact);

						//FVector right_cross_cs = FVector::CrossProduct(character_forward_direction_vector_CS,character_direction_vector_CS);

						//FVector right_cross = UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), right_cross_cs);
					...
					//	FTransform ComponentBoneTransform_Finger = MeshBasesSaved.Pose.GetComponentSpaceTransform(ModifyBoneIndex_Local_Finger);
						...
						if (Ignore_Lerping == false)
							feet_fingers_transform_array[spine_index][feet_index][fingerx].SetRotation(AnimRotSLerp(feet_fingers_transform_array[spine_index][feet_index][fingerx].GetRotation(), Diff_Vect.GetRotation(), (1 - FMath::Exp(-10 * owning_skel->GetWorld()->GetDeltaSeconds())) * feet_rotation_speed));
						else
							feet_fingers_transform_array[spine_index][feet_index][fingerx].SetRotation(ComponentBoneTransform_Finger.GetRotation());
...
FRotator FAnimNode_DragonFeetSolver::RotationFromImpactNormal(int spine_index,int feet_index,bool is_finger,FComponentSpacePoseContext& Output, FVector normal_impact_input, FTransform original_Bone_transform, float feet_limit)
{
	...
	FVector normal_impact = UKismetMathLibrary::InverseTransformDirection(owning_skel->GetComponentToWorld(), normal_impact_input);
	...
	//	FAnimationRuntime::ConvertCSTransformToBoneSpace(owning_skel->GetComponentToWorld(), Output.Pose, TestTransform1, spine_Feet_pair[spine_index].Associated_Feet[feet_index].CachedCompactPoseIndex,EBoneControlSpace::BCS_ComponentSpace);
	...
	//	FAnimationRuntime::ConvertBoneSpaceTransformToCS(owning_skel->GetComponentToWorld(), Output.Pose, TestTransform1, spine_Feet_pair[spine_index].Associated_Feet[feet_index].CachedCompactPoseIndex, EBoneControlSpace::BCS_ComponentSpace);
...
void FAnimNode_DragonFeetSolver::UpdateInternal(const FAnimationUpdateContext& Context)
{
	...
	if (owning_skel)
	{
		current_fps = 1 / (0.01f+owning_skel->GetWorld()->DeltaTimeSeconds);
	...
	Delta_Loc_Speed = FMath::Clamp<float>(owning_skel->GetWorld()->DeltaTimeSeconds * 15 * Location_Lerp_Speed * extra_multiplier, 0, 1);


	Delta_Rot_Speed = FMath::Clamp<float>((1 - FMath::Exp(-10 * owning_skel->GetWorld()->GetDeltaSeconds())) * feet_rotation_speed * extra_multiplier, 0, 1);


	//	Delta_Loc_Speed = owning_skel->GetWorld()->DeltaTimeSeconds * 15 * Location_Lerp_Speed;
	...
	if (enable_solver)
	{
		...
		if (test_counter < 10)
		{
			Delta_Loc_Speed = FMath::Clamp<float>(owning_skel->GetWorld()->DeltaTimeSeconds * 100, 0, 1);
		}
		...
		FVector character_forward_vector = UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), character_forward_direction_vector_CS);
				...
				if (i < spine_Feet_pair.Num() && i < spine_Transform_pairs.Num())
					for (int32 j = 0; j < spine_Feet_pair[i].Associated_Feet.Num(); j++)
					{
						...
						if (j < spine_Transform_pairs[i].Associated_Feet.Num() && dragon_input_data.FeetBones.Num() > spine_Feet_pair[i].order_index[j])
						{
							...
							FVector foot_offset_a = UKismetMathLibrary::TransformLocation(owning_skel->GetComponentToWorld(), dragon_input_data.FeetBones[spine_Feet_pair[i].order_index[j]].Feet_Trace_Offset);							
							FVector foot_offset_b = UKismetMathLibrary::TransformLocation(owning_skel->GetComponentToWorld(), FVector(0,0,0));

							FVector feet_offset_difference = foot_offset_a - foot_offset_b;

							offseted_linetrace_location += feet_offset_difference;

						//	offseted_linetrace_location += (UKismetMathLibrary::TransformLocation(owning_skel->GetComponentToWorld(), dragon_input_data.FeetBones[spine_Feet_pair[i].order_index[j]].Feet_Trace_Offset));
							...
							FeetTipLocations[i][j] = UKismetMathLibrary::InverseTransformLocation(owning_skel->GetComponentToWorld(), Front_point_location);
							...
							*/
							...
							//	FVector finger_loc = owning_skel->GetComponentToWorld().TransformPosition( feet_fingers_transform_array[i][j][k_finger].GetLocation() + trace_offset);

								FVector finger_loc = owning_skel->GetComponentToWorld().TransformPosition(feet_fingers_transform_array[i][j][k_finger].GetLocation() + trace_offset);



								FVector original_finger_loc = owning_skel->GetComponentToWorld().TransformPosition(feet_fingers_transform_array[i][j][k_finger].GetLocation() + FVector(0, 0, trace_offset.Z));
...
void FAnimNode_DragonFeetSolver::Leg_ik_Function(FBoneReference ik_footbone, int spine_index, int feet_index, TEnumAsByte<enum EBoneControlSpace> EffectorLocationSpace, TEnumAsByte<enum EBoneControlSpace> JointTargetLocationSpace, FComponentSpacePoseContext& MeshBasesSaved, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if (!first_time_setup && atleast_one_hit && enable_solver == true)
	{
		...
		//	FVector FeetImpactPoint = spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].ImpactPoint;

		//	if (!feet_ishit_array[spine_index][feet_index])
		//	if(Original_EndBoneCSTransform.GetLocation().Z > 1)
			{
				//	feet_impactpoint_array[spine_index][feet_index] = spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].ImpactPoint;
				...
				{
					//	Temp_Impact_Ref = UKismetMathLibrary::TransformLocation(owning_skel->GetComponentToWorld(), Temp_Impact_Ref);
					//	Temp_Impact_Ref.Z = 0;
					//	Temp_Impact_Ref = UKismetMathLibrary::InverseTransformLocation(owning_skel->GetComponentToWorld(), Temp_Impact_Ref);

					Temp_Impact_Ref.Z = (eb_wtransform.GetLocation() - character_direction_vector * FeetRootHeights[spine_index][feet_index]).Z;
				}
				//	feet_impactpoint_array[spine_index][feet_index] = UKismetMathLibrary::VLerp(FVector(Temp_Impact_Ref.X, Temp_Impact_Ref.Y,feet_impactpoint_array[spine_index][feet_index].Z), Temp_Impact_Ref,owning_skel->GetWorld()->DeltaTimeSeconds * 15 * Location_Lerp_Speed);
				...
				//	feet_impactpoint_array[spine_index][feet_index] = AnimLocLerp(spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].bBlockingHit, spine_index, feet_index, feet_impactpoint_array[spine_index][feet_index], Temp_Impact_Ref, owning_skel->GetWorld()->DeltaTimeSeconds * 15 * Location_Lerp_Speed);


			//	feet_impactpoint_array[spine_index][feet_index] = UKismetMathLibrary::VLerp(feet_impactpoint_array[spine_index][feet_index], Temp_Impact_Ref, owning_skel->GetWorld()->DeltaTimeSeconds * 15 * Location_Lerp_Speed);


			//	FVector Fixed_Impact_Lerp = FVector(spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].ImpactPoint.X, spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].ImpactPoint.Y, feet_impactpoint_array[spine_index][feet_index].Z);

			//	feet_impactpoint_array[spine_index][feet_index] = AnimLocLerp(spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].bBlockingHit, spine_index, feet_index, feet_impactpoint_array[spine_index][feet_index], Fixed_Impact_Lerp, owning_skel->GetWorld()->DeltaTimeSeconds * 15 * Location_Lerp_Speed);




				//feet_impactpoint_array[spine_index][feet_index] = FVector(EndBoneCSTransformX.GetLocation().X, EndBoneCSTransformX.GetLocation().Y, EndBoneCSTransformX.GetLocation().Z);



		//		feet_ishit_array[spine_index][feet_index] = true;
			}



			/*
			FeetImpactPoint = owning_skel->GetComponentTransform().InverseTransformPosition(FeetImpactPoint);

			for (int fingerx = 0; fingerx < spine_hit_pairs[spine_index].RV_Associated_Fingers_Hits[feet_index].Num(); fingerx++)
			{

				FVector fingerpoint = spine_hit_pairs[spine_index].RV_Associated_Fingers_Hits[feet_index][fingerx].ImpactPoint;
			//	fingerpoint = owning_skel->GetComponentTransform().InverseTransformPosition(fingerpoint);

				if (fingerpoint.Z > FeetImpactPoint.Z)
				{
					FeetImpactPoint.Z = fingerpoint.Z;
				}

			}

			FeetImpactPoint = owning_skel->GetComponentTransform().TransformPosition(FeetImpactPoint);
			*/
			...
			//	EffectorLocation_Point = (FeetImpactPoint + spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].Normal * FeetRootHeights[spine_index][feet_index]);
			...
			//	FVector Formatted_Effector_Point = UpperLimbCSTransform.GetLocation() + Effector_Thigh_Dir* FMath::Clamp<float>(Effector_Thigh_Size, FMath::Clamp<float>(Effector_Thigh_Size - FMath::Abs(dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Max_Feet_Float),0,10000000), Effector_Thigh_Size);


			//	FVector Formatted_Effector_Point = UpperLimbCSTransform.GetLocation() + Effector_Thigh_Dir * FMath::Clamp<float>(Point_Thigh_Size, Effector_Thigh_Size - FMath::Abs(dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Max_Feet_Float), Point_Thigh_Size*2);

			//	FVector Formatted_Effector_Point = UpperLimbCSTransform.GetLocation() + Effector_Thigh_Dir * FMath::Clamp<float>(Point_Thigh_Size, UKismetMathLibrary::Max(Effector_Thigh_Size - FMath::Abs(dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Max_Feet_Float),0 ), Effector_Thigh_Size);

			//	FVector Formatted_Effector_Point = UpperLimbCSTransform.GetLocation() + Effector_Thigh_Dir * Point_Thigh_Size;
			...
			//	EffectorLocation_Point.Z = FMath::Clamp<float>(EffectorLocation_Point.Z, EndBoneCSTransform.GetLocation().Z - FMath::Abs(dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Max_Feet_Float), EndBoneCSTransform.GetLocation().Z + FMath::Abs(dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Max_Feet_Float * scale_mode) );

			//	EffectorLocation_Point.Z = FMath::Clamp<float>(EffectorLocation_Point.Z, EndBoneCSTransform.GetLocation().Z - FMath::Abs(dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Max_Feet_Float), FMath::Abs(dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Max_Feet_Float * scale_mode));

				//EffectorLocation_Point.Z = Formatted_Effector_Point.Z;
		...
		//FTransform EffectorPointTransform_Around = FTransform::Identity;
		//EffectorPointTransform_Around.SetLocation(EffectorLocation_Point);

		//FTransform EffectorPointTransform_Diff = FTransform::Identity;
		//EffectorPointTransform_Diff.SetRotation(Rotated_Difference);

		//GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, " Feet Rotated_Difference : " + FRotator(Rotated_Difference).ToString());
		//FTransform Rotated_Foot_Transform = EffectorPointTransform_Diff*EffectorPointTransform_Around;
		//FTransform Rotated_Foot_Transform = EffectorPointTransform_Around * Rotated_Difference;


		//feet_mod_transform_array[spine_index][feet_index].SetLocation(Rotated_Transform.GetLocation());
		...
		/*
		float Feet_Roll = FRotator(Rotated_Difference).Roll;

		if(Feet_Roll > 0)
		EffectorLocation_Point.Z += FMath::Abs(Feet_Roll) * dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Feet_Slope_Offset_Multiplier * scale_mode;
		else
			EffectorLocation_Point.Z += FMath::Abs(Feet_Roll) * 5 * scale_mode;
			*/

		//

		/*
		FVector Thigh_Feet_Dir = (EffectorLocation_Point - UpperLimbCSTransform.GetLocation()).GetSafeNormal();
		float Thigh_Feet_Len = (EffectorLocation_Point - UpperLimbCSTransform.GetLocation()).Size();

		FVector Down_Vector = character_direction_vector_CS * -1;


		EffectorLocation_Point = UpperLimbCSTransform.GetLocation() + (UKismetMathLibrary::VLerp(Down_Vector, Thigh_Feet_Dir, speed_warping_const) * Thigh_Feet_Len);
		*/
		...
		/*
			...
			{                            //UKismetMathLibrary::VLerp
				feet_mod_transform_array[spine_index][feet_index].SetLocation(AnimLocLerp(spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].bBlockingHit,spine_index, feet_index, feet_mod_transform_array[spine_index][feet_index].GetLocation(), EffectorLocation_Point, owning_skel->GetWorld()->DeltaTimeSeconds * 15 * Location_Lerp_Speed));
			}
			else*/
		{
			feet_mod_transform_array[spine_index][feet_index].SetLocation(EffectorLocation_Point);
		...
		if (spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].bBlockingHit || first_time_count < 3)
			feet_Alpha_array[spine_index][feet_index] = UKismetMathLibrary::FInterpTo(feet_Alpha_array[spine_index][feet_index], dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Feet_Alpha, 1 - FMath::Exp(-10 * owning_skel->GetWorld()->GetDeltaSeconds()), shift_speed * 0.5f);
		else
	...
	//else
	{
		...
		//	FTransform eb_wtransform = EndBoneCSTransform;
			...
			if (Ignore_Location_Lerping == false && (feet_mod_transform_array[spine_index][feet_index].GetLocation() - eb_wtransform.GetLocation()).Size() < 500)
				feet_mod_transform_array[spine_index][feet_index].SetLocation(AnimLocLerp(spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].bBlockingHit, spine_index, feet_index, feet_mod_transform_array[spine_index][feet_index].GetLocation(), eb_wtransform.GetLocation(), owning_skel->GetWorld()->DeltaTimeSeconds * 15 * Location_Lerp_Speed));
			else*/
		{

			//	feet_impactpoint_array[spine_index][feet_index] = UKismetMathLibrary::VLerp(feet_impactpoint_array[spine_index][feet_index], owning_skel->GetComponentToWorld().TransformPosition(eb_wtransform.GetLocation()) - character_direction_vector * FeetRootHeights[spine_index][feet_index], owning_skel->GetWorld()->DeltaTimeSeconds * 15 * Location_Lerp_Speed);


			//	FVector feet_world_vec = owning_skel->GetComponentToWorld().TransformPosition(eb_wtransform.GetLocation()) - character_direction_vector * FeetRootHeights[spine_index][feet_index];
			//	FVector Fixed_Impact_Lerp = FVector(feet_world_vec.X, feet_world_vec.Y, feet_impactpoint_array[spine_index][feet_index].Z);
			//	feet_impactpoint_array[spine_index][feet_index] = UKismetMathLibrary::VLerp(feet_impactpoint_array[spine_index][feet_index], feet_world_vec, owning_skel->GetWorld()->DeltaTimeSeconds * 15 * Location_Lerp_Speed);


			//	feet_impactpoint_array[spine_index][feet_index] = UKismetMathLibrary::VLerp(feet_impactpoint_array[spine_index][feet_index], spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].ImpactPoint, owning_skel->GetWorld()->DeltaTimeSeconds * 15 * Location_Lerp_Speed);

			//	feet_impactpoint_array[spine_index][feet_index] = owning_skel->GetComponentToWorld().TransformPosition(eb_wtransform.GetLocation()) - character_direction_vector * FeetRootHeights[spine_index][feet_index];
		...
		//	feet_ishit_array[spine_index][feet_index] = false;
...
//	FRP_Knee_Transform.SetLocation(dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Knee_Direction_Offset);
	...
	BendingDirectionTransform = BendingDirectionTransform* FRP_Knee_Transform;
	*/


	//ORIGINAL BendingDirectionTransform.SetLocation(BendingDirectionTransform.GetLocation() + cs_forward *-1000);
...

	{
		...
		else
		{
			...
			if (Ignore_Lerping == false)
				feet_mod_transform_array[spine_index][feet_index].SetRotation(UKismetMathLibrary::RLerp(feet_mod_transform_array[spine_index][feet_index].Rotator(), EndBoneCSTransform.Rotator(), FMath::Clamp<float>(owning_skel->GetWorld()->DeltaTimeSeconds * feet_rotation_speed * 5, 0, 1), true).Quaternion());
			else
				feet_mod_transform_array[spine_index][feet_index].SetRotation(EndBoneCSTransform.GetRotation());
	...
	if (Affect_Toes_Always)
	{
		...
		for (int fingerx = 0; fingerx < spine_hit_pairs[spine_index].RV_Associated_Fingers_Hits[feet_index].Num(); fingerx++)
		{
			...
			else
			{
				...
				FVector impact_finger_point = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[spine_index].RV_Associated_Fingers_Hits[feet_index][fingerx].ImpactPoint + FVector(0,0,10));
				...
				for (int32 j = 0; j < spine_Feet_pair[i].Associated_Feet.Num(); j++)
				{
					...
					if (j < spine_Transform_pairs[i].Associated_Feet.Num())
					{
						...
						for (int32 finger_k = 0; finger_k < spine_Transform_pairs[i].Associated_Fingers[j].Num(); finger_k++)
						{
							...
							spine_Transform_pairs[i].Associated_Fingers[j][finger_k] = ComponentBoneTransform_Finger * owning_skel->GetComponentTransform();
						}
